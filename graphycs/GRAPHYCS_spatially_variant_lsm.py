...
from forward_model_lsm_variant import forward_model_lsm_variant
...
if __name__=="__main__":
    ...
    parser.add_argument("--spatially_varying_weight", default=1e-3, type=float)
    ...
    parser.add_argument("--vis_frequency", default=100, type=int)
    parser.add_argument("--use_L1_loss", default=1, type=int)
    parser.add_argument("--sparsity_weight", default=1.0, type=float)


    parser.add_argument("--use_batch_shuffling", default=0, type=int)
    parser.add_argument("--num_patches_h", default=10, type=int)
    parser.add_argument("--num_patches_w", default=8, type=int)
    ...
    coeff_batch = dset.coeffs
    diversity_img_batch = torch.stack(dset.ys, axis=0)


    print("Coeff batches shape:", coeff_batch.shape
          , "Diversity image batches shape:", diversity_img_batch.shape)
    ...
    print("coeff batches shape after removing the first batch:", coeff_batch.shape
          , "diversity image batches shape after removing the first batch:", diversity_img_batch.shape)
    assert coeff_batch.shape[0] == diversity_img_batch.shape[0], "Coefficient batches and diversity image batches must have the same number of elements."
 
    ks = opt.psf_size
    pad_size = ks -1
    pad_w, pad_h = pad_size, pad_size

    model = forward_model_lsm_variant(original_img, diversity_img_batch, opt.num_patches_h, opt.num_patches_w, 14, ks, M, wavelength, camera_pixel_size, opt.n_imm, opt.NA, len(coeff_batch) - 1, device, 
    ...
    param_groups = [
        ...
        {'params': [model.varying_zernike_coef], 'lr': opt.varying_coef_lr},
    ...
    _, w_d, h_d = diversity_img_batch.shape

    for epoch_idx in t:
        if opt.use_batch_shuffling == 1:
            idxs = torch.randperm(len(diversity_img_batch)).long().to(device)
        else:
            idxs = torch.arange(len(diversity_img_batch)).long().to(device)


        optimizer.zero_grad()
        model.estimated_obj.requires_grad_()
        model.zernike_coef.requires_grad_()
        model.varying_zernike_coef.requires_grad_()
        ...
        for it in range(0, len(diversity_img_batch), opt.batch_size):
            idx = idxs[it:min(it + opt.batch_size, len(diversity_img_batch))]
            coeffs, y_batch = coeff_batch[idx].to(device), diversity_img_batch[idx].to(device)
            ...
            if opt.use_L1_loss == 1:
                ...
                recon_loss = L1_loss(aberrated_imgs.squeeze(), y_batch_cropped.squeeze())
                Fourier_loss = mse_loss(recon_observed_imgs_FT.squeeze(), observed_imgs_FT.squeeze())
                recon_loss = mse_loss(aberrated_imgs.squeeze(), y_batch_cropped.squeeze())

            sparsity_loss = torch.norm(model.activity_factors, p=1) / model.activity_factors.numel() if model.learn_activity else 0.0
            L1_regularization = torch.norm(model.varying_zernike_coef, p=1) / model.varying_zernike_coef.numel()

            epoch_image_loss += recon_loss.item()
            epoch_fourier_loss += Fourier_loss.item()
            loss = recon_loss + opt.fourier_loss_weight * Fourier_loss + opt.sparsity_weight * sparsity_loss + opt.spatially_varying_weight * L1_regularization
            ...
            if (epoch_idx % opt.vis_frequency) == 0 and it == 0:
                ...
                if opt.vis_intermediates == 1:
                    ...
                    estimated_activity = model.activity_factors.squeeze().detach().cpu().numpy()
                    skio.imsave(os.path.join(base_dir, "{}/found_objects/estimated_activity_{}.tif".format(exp_name, epoch_idx)), estimated_activity.astype(np.float32), imagej=True)

                    combined_estimated_object = model.estimated_obj.squeeze().unsqueeze(0).detach().cpu().numpy() + estimated_activity
                    skio.imsave(os.path.join(base_dir, "{}/found_objects/estimated_object_w_activity{}.tif".format(exp_name, epoch_idx)), combined_estimated_object.astype(np.float32), imagej=True)

                found_object_stack.append(model.estimated_obj.detach().cpu().numpy())

                psfs_squeezed = psfs.squeeze().detach().cpu()
                iPSF_stack.append(psfs_squeezed[0,int((opt.num_patches_h * opt.num_patches_w // 2)), ...].numpy())

                psfs_squeezed_vis = psfs_squeezed[0,...].unsqueeze(0)  # Add batch dimension
                _, _, h, w = psfs_squeezed_vis.shape
                psfs_squeezed_vis = psfs_squeezed_vis.flatten(2,3)
                psfs_squeezed_vis = psfs_squeezed_vis.permute(0, 2, 1)  # (B, H*W, C)
                psfs_squeezed_vis = F.fold(psfs_squeezed_vis, (opt.num_patches_h * h, opt.num_patches_w * w), (h, w), stride=(h, w))  # Reshape back to (B, C, H, W)
                psfs_squeezed_vis = psfs_squeezed_vis.squeeze().numpy()
                skio.imsave(base_dir + "/{}/iPSFs/psfs_sv_{}.tif".format(exp_name, epoch_idx), psfs_squeezed_vis)
            ...
            model.varying_zernike_coef.grad.mul_(mask) 
            ...
            del y_batch, coeffs, aberrated_imgs, psfs, recon_observed_imgs_FT, observed_imgs_FT
        ...
        if opt.use_motion_estimation:
            motion_factors_norm_ep.append(torch.norm(model.motion_factors.detach()).item())
        ...
        zernikes_varying_L1_ep.append(L1_regularization.item())
        ...
        if epoch_idx % opt.vis_frequency == 0:
            ...
            np.save(base_dir + "/{}/sparsity_loss.npy".format(exp_name), np.array(activity_sparsity_loss_ep))
            np.save(base_dir + "/{}/zernikes_varying_L1.npy".format(exp_name), np.array(zernikes_varying_L1_ep))
            np.save(base_dir + "/{}/motion_factors_norm.npy".format(exp_name), np.array(motion_factors_norm_ep))
        ...
        if epoch_idx == opt.epochs - 1:
            ...
            np.save(base_dir + "/{}/final_results/activity_sparsity_loss_ep.npy".format(exp_name), np.array(activity_sparsity_loss_ep))
            ...
            with torch.no_grad():
                ...
                beam_scaled = scale_transform(beam_crop, model.amp_x.detach(), model.amp_y.detach(), device)
                beam_scaled_shifted = shift_transform(beam_scaled, model.off_x.detach(), model.off_y.detach(), device)
                phase_estimated_vis = beam_scaled_shifted * estimated_phase
        ...
        if epoch_idx % opt.vis_frequency == 0:
            ...
            # Convert estimated_obj to CHW
            img_obj = model.estimated_obj.detach().cpu()
            if img_obj.ndim == 2:  # (H, W)
                img_obj = img_obj.unsqueeze(0)  # (1, H, W)
            elif img_obj.ndim == 3:  # (B, H, W) -> take one sample
                img_obj = img_obj[0].unsqueeze(0)  # (1, H, W)
            img_phase = estimated_phase.detach().cpu()
            if img_phase.ndim == 2:
                img_phase = img_phase.unsqueeze(0)  # (1, H, W)
            model.eval()
            with torch.no_grad():
                shift_scaled_amplitude = amplitude_transformed.detach().cpu().squeeze()
                shift_scaled_amplitude = shift_scaled_amplitude.unsqueeze(0)  # (1, H, W)
        ...
        if epoch_idx % opt.vis_frequency == 0:
            ...
            np.savetxt(base_dir + "/{}/zernikes_saved/varying_zernikes_{}.txt".format(exp_name, epoch_idx), np.array(model.varying_zernike_coef.detach().cpu().numpy()))
            ...
            plt.subplot(2,2,3)
            plt.plot(activity_sparsity_loss_ep)
            plt.title("Activity L1 Norm", fontdict={'fontsize': 12})
            plt.xlabel("epochs")
            plt.ylabel("loss")

            plt.subplot(2,2,4)
            plt.plot(zernikes_varying_L1_ep)
            plt.title("Spatially Varying Zernikes L1 Norm", fontdict={'fontsize': 12})
            plt.xlabel("epochs")
            plt.ylabel("loss")
            ...
            if opt.use_motion_estimation:
                motion_factors_vis = model.motion_factors.detach().cpu().numpy()
                np.savetxt(base_dir + "/{}/motion_factors/motion_factors_{}.txt".format(exp_name, epoch_idx), motion_factors_vis)

                # motion_factors_labels = ['Motion Factor {}'.format(i) for i in range(motion_factors_vis.shape[0])]

                plt.figure(figsize=(10,6))
                plt.plot(motion_factors_norm_ep)
                plt.xlabel("epochs")
                plt.ylabel("motion factors norm")
                plt.title("Motion Factors L2 Norm", fontdict={'fontsize': 12})
                plt.tight_layout()
                plt.savefig(base_dir + "/{}/motion_factors_norm.png".format(exp_name))
                plt.clf()
                plt.close()
            ...
            #### visualization for spatially varying residual phases:
            spatially_varying_residual_phases = torch.einsum('ni,ijk->njk', model.varying_zernike_coef.detach(), zernikes)
            spatially_varying_residual_phases = (2 * np.pi * opt.n_imm / wavelength) * 1e-6 * spatially_varying_residual_phases
            spatially_varying_residual_phases[beam_crop.unsqueeze(0).repeat(int(opt.num_patches_h * opt.num_patches_w), 1, 1) == 0] = torch.tensor(float('nan')).to(device)  # Set NaN where beam_crop is 0
            spatially_varying_residual_phases = spatially_varying_residual_phases.unsqueeze(0)  # Add batch dimension
            _, _, h, w = spatially_varying_residual_phases.shape
            spatially_varying_residual_phases = spatially_varying_residual_phases.flatten(2,3)
            spatially_varying_residual_phases = spatially_varying_residual_phases.permute(0, 2, 1)  # (B, H*W, C)
            spatially_varying_residual_phases = F.fold(spatially_varying_residual_phases, (opt.num_patches_h * h, opt.num_patches_w * w), (h, w), stride=(h, w))  # Reshape back to (B, C, H, W)

            ticks = [-1 * np.pi, 0, 1 * np.pi]
            labels = [r'$-\pi$', '0', r'$\pi$']
            fig_sv, ax = plt.subplots(figsize=(20, 20))
            im = ax.imshow(spatially_varying_residual_phases.squeeze().cpu().numpy(), cmap='jet', vmin=-1 * np.pi, vmax=1 * np.pi)
            cbar = fig_sv.colorbar(im, ax=ax, shrink=0.3)
            cbar.ax.tick_params(labelsize=18)
            cbar.set_ticks(ticks)
            # cbar.ax.set_title("lmbda", fontsize=18, pad=10)
            cbar.set_ticklabels(labels)
            ax.axis('off')
            ax.set_title(f"Residual Phases for Different Patches", fontsize=30)
            fig_sv.savefig(base_dir + "/{}/phase_estimations/spatially_varying_residual_phases_{}.jpg".format(exp_name, epoch_idx), dpi=300)
            plt.close()
