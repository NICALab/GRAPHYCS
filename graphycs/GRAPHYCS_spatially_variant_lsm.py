        ...
        if epoch_idx % opt.vis_frequency == 0:
            ...
            save_loss_curve_visualization(
                ...
                [
                    ...
                    ("Spatially varying Zernike L1 norm", zernikes_varying_L1_ep),
                ])
            ...
            spatial_phase_meters = 1e-6 * torch.einsum('ni,ijk->njk', model.varying_zernike_coef.detach(), zernikes)
            spatial_mask = beam_crop.unsqueeze(0).repeat(int(opt.num_patches_h * opt.num_patches_w), 1, 1).bool()
            spatial_phase_meters = spatial_phase_meters.masked_fill(~spatial_mask, torch.tensor(float('nan'), device=device))
            spatial_phase_meters = spatial_phase_meters.unsqueeze(0)
            _, _, h_sv, w_sv = spatial_phase_meters.shape
            spatial_phase_grid = spatial_phase_meters.flatten(2, 3)
            spatial_phase_grid = spatial_phase_grid.permute(0, 2, 1)
            spatial_phase_grid = F.fold(spatial_phase_grid, (opt.num_patches_h * h_sv, opt.num_patches_w * w_sv), (h_sv, w_sv), stride=(h_sv, w_sv))
            spatial_phase_grid_wrapped = wrap_phase_minus_pi_pi_for_visualization(
                spatial_phase_grid.squeeze(), wavelength, opt.n_imm)
            save_phase_map_visualization(
                base_dir + "/{}/phase_estimations/spatially_varying_phase_epoch_{:06d}.png".format(exp_name, epoch_idx),
                spatial_phase_grid_wrapped,
                title='Spatially varying phase components ({}x{} grid)'.format(opt.num_patches_h, opt.num_patches_w))
