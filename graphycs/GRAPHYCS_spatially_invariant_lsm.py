            ...
            if (epoch_idx % opt.vis_frequency) == 0 and it == 0:
                ...
                if opt.vis_intermediates == 1:
                    ...
                    with torch.no_grad():
                        ...
                        skio.imsave(base_dir + "/{}/found_objects/estimated_activity_{}.tif".format(exp_name, epoch_idx), np.array(estimated_activity))
                        combined_estimated_object = model.estimated_obj.squeeze().unsqueeze(0).detach().cpu().numpy() + estimated_activity
                        skio.imsave(base_dir + "/{}/found_objects/estimated_object_w_activity{}.tif".format(exp_name, epoch_idx), np.array(combined_estimated_object))
