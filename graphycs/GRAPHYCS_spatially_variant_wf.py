...
from forward_model_wf_variant import forward_model_wf_variant
...
if __name__=="__main__":
    ...
    parser.add_argument("--use_flat_field_correction", default=1, type=int)
    ...
    parser.add_argument("--spatially_varying_weight", default=0.002, type=float)
    ...
    model = forward_model_wf_variant(original_img, 14, ks, opt.num_patches, M, wavelength, camera_pixel_size, opt.n_imm, opt.NA, len(coeff_batches) - 1, device, 
                                          learn_affine_transform, learn_scales)
    
    param_groups = [
        {'params': [model.estimated_obj], 'lr': opt.object_lr},
        {'params': [model.zernike_coef], 'lr': opt.lr},
        {'params': [model.varying_zernike_coef], 'lr': opt.lr},
    ...
    for epoch_idx in t:
        ...
        for it in range(0, len(dset), opt.batch_size):
            ...
            loss = recon_loss + opt.fourier_loss_weight * Fourier_loss + opt.spatially_varying_weight * L1_regularization
            ...
            if (epoch_idx % opt.vis_frequency) == 0 and it == 0:
                ...
                iPSF_stack.append(psfs_squeezed[0,int((opt.num_patches * opt.num_patches // 2)), ...].numpy())

                psfs_squeezed_vis = psfs_squeezed[0,...].unsqueeze(0)  # Add batch dimension

                if psfs_squeezed_vis.ndim == 4:
                    ...
                    psfs_squeezed_vis = F.fold(psfs_squeezed_vis, (opt.num_patches * h, opt.num_patches * w), (h, w), stride=(h, w))  # Reshape back to (B, C, H, W)
                    skio.imsave(base_dir + "/{}/iPSFs/psfs_sv_{}.tif".format(exp_name, epoch_idx), psfs_squeezed_vis.squeeze().numpy())
                else:
                    psf_save = psfs.detach().squeeze()
                    _, h, w = psf_save.shape
                    # import pdb; pdb.set_trace();
                    psf_save = psf_save.flatten(1,2)
                    psf_save = psf_save.permute(1, 0)  # (H*W, C)
                    psf_save = F.fold(psf_save, (opt.num_patches * h, opt.num_patches * w), (h, w), stride=(h, w))  # Reshape back to (B, C, H, W)
                    skio.imsave(base_dir + "/{}/iPSFs/psfs_sv_{}.tif".format(exp_name, epoch_idx), psf_save.squeeze().cpu().numpy())
            ...
            del aberrated_imgs, psfs, y_batch, coeffs
        ...
        if epoch_idx == opt.epochs - 1:
            ...
            ## save all stn params:
            params_save = [model.amp_x.item(), model.amp_y.item(), model.off_x.item(), model.off_y.item(), model.rot.item(), model.grid.item()]
        ...
        if epoch_idx % opt.vis_frequency == 0:
            ...
            plt.title("L1 regularization on Varying Zernikes", fontdict={'fontsize': 12})
            plt.xlabel("epochs")
            plt.ylabel("loss")
            ...
            spatially_varying_residual_phases[beam_crop.unsqueeze(0).repeat(int(opt.num_patches * opt.num_patches), 1, 1) == 0] = torch.tensor(float('nan')).to(device)  # Set NaN where beam_crop is 0
            ...
            spatially_varying_residual_phases = F.fold(spatially_varying_residual_phases, (opt.num_patches * h, opt.num_patches * w), (h, w), stride=(h, w))  # Reshape back to (B, C, H, W)

            ticks = [-1 * np.pi, 0, 1 * np.pi]
            labels = [r'$-\pi$', '0', r'$\pi$']
            fig_sv, ax = plt.subplots(figsize=(20, 20))
            im = ax.imshow(spatially_varying_residual_phases.squeeze().cpu().numpy(), cmap='jet', vmin=-np.pi, vmax=np.pi)
