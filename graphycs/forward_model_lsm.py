...
class forward_model_lsm(nn.Module):
    def __init__(self,
        ...
        self.activity_factors = phase_added_images.clone().to(device) - init_object.clone().to(device)  # [N, H, W]
    ...
    def forward(self, added_coeffs, indices):
        ...
        # added_coeffs = added_coeffs.squeeze()  # [N, n_max+1]
        if self.use_scales:
            ...
            added_coeffs = added_coeffs * scale_factors.repeat(added_coeffs.shape[0], 1)  # [N, n_max+1]
        ...
        if self.learn_motion:
            # apply motion factors to the PSF
            if out.ndim == 2:
                out = out.unsqueeze(0)
            assert out.ndim == 3, "Estimated object must be of shape (1, H,W) or (N,H,W)"
            out = shift_transform_batch(out.unsqueeze(1), self.motion_factors[indices, 0], self.motion_factors[indices, 1], device=self.device)
            out = out.squeeze(1)
            _, w, h = out.shape
            out = out[:, 10:w-10, 10:h-10]  # crop the output to remove padding
