...
def affine_transform(gaussian_intensity_large, scale_factor_x, scale_factor_y, offset_x_learned, offset_y_learned):
    device = gaussian_intensity_large.device
    dtype = gaussian_intensity_large.dtype
    max_offset = torch.tensor([0.2], device=device, dtype=dtype)
    min_offset = torch.tensor([-0.2], device=device, dtype=dtype)
    zero = torch.tensor([0.0], device=device, dtype=dtype)

    scale_factor_x = scale_factor_x.to(device=device, dtype=dtype)
    scale_factor_y = scale_factor_y.to(device=device, dtype=dtype)
    offset_x_learned = offset_x_learned.to(device=device, dtype=dtype)
    offset_y_learned = offset_y_learned.to(device=device, dtype=dtype)

    offset_x_learned = torch.max(torch.min(offset_x_learned, max_offset), min_offset)
    offset_y_learned = torch.max(torch.min(offset_y_learned, max_offset), min_offset)

    theta_tilt = torch.stack([
        torch.cat([scale_factor_x, zero, offset_x_learned]),
        torch.cat([zero, scale_factor_y, offset_y_learned])
    ]).unsqueeze(0)
    gaussian_intensity_large = torch.unsqueeze(torch.unsqueeze(gaussian_intensity_large, 0), 0)
    grid = F.affine_grid(theta_tilt, gaussian_intensity_large.size(), align_corners=False)
    gaussian_intensity_large = F.grid_sample(gaussian_intensity_large, grid, align_corners=False)
