...
if __name__=="__main__":
    ...
    parser.add_argument("--exp_name", default='2n+1_0.5_zernikes4_11', type=str)
    ...
    parser.add_argument("--data_path", default="D:\\CAO_Data_2025\\20250304_spatially_varying_test\\Data\\CALIB_sys_defocus_250226_Lymph_SV_S1R1\\CALIB_sys_defocus_250226_Lymph_SV_S1R1_Z5.tif", type=str) # CALIB_250107_SomaGCaMP7f2dpf_S1R1_crop CALIB_250107_Lymph_S1R1_crop
    ...
    parser.add_argument("--use_STN", default=1, type=int) # 1, 0
    parser.add_argument("--use_learnable_scales", default=0, type=bool) # 1,0
    ...
    parser.add_argument("--num_patches_h", default=4, type=int)
    parser.add_argument("--num_patches_w", default=4, type=int)
    parser.add_argument("--spatially_varying_weight", default=0.01, type=float)
    ...
    os.makedirs(base_dir + "/{}/zernikes_saved/common_mode".format(exp_name), exist_ok=True)
    os.makedirs(base_dir + "/{}/zernikes_saved/varying".format(exp_name), exist_ok=True)
    ...
    # _, zernikes = zernike_pd_generation_higher_order(6, max_zernike_idx_ansi, M, camera_pixel_size, wavelength, num_apert, 1)
    ...
    ####### Settings for the Chirped Z transform:
    ...
    print(coeffs.shape)
    cols_to_add = opt.n_max_estimated + 1 - coeffs.shape[1]
    coeffs = np.pad(coeffs, ((0, 0), (0, cols_to_add)), mode='constant', constant_values=0)
    ...
    # phase_added_images = np.maximum(phase_added_images - opt.avg_background, np.zeros_like(phase_added_images))
    ...
    # phase_added_images = phase_added_images[:, 8:511-8, 8:511 - 8]
    phase_added_images = phase_added_images / np.max(phase_added_images)

    ## sample a subset of the coefficients and the images according to the img_num_factor
    ...
    num_patches_w, num_patches_h = opt.num_patches_w, opt.num_patches_h
    ks = opt.psf_size
    pad_size = ks -1
    pad_w, pad_h = pad_size, pad_size
    ## divide using overlap_save method
    patch_h = img_h // num_patches_h + pad_size
    stride_h = img_h // num_patches_h
    patch_w = img_w // num_patches_w + pad_size
    stride_w = img_w // num_patches_w
    ...
    found_zernike_coef_diff_fovs = torch.zeros((opt.num_patches_h*opt.num_patches_w, opt.n_max + 1), dtype=torch.float32).to(device)
    # found_zernike_coef_diff_fovs = 0.05 * torch.rand((opt.num_patches_h*opt.num_patches_w, opt.n_max + 1), dtype=torch.float32).to(device)
    ...
    stn_params = [amplitude_scale_x, amplitude_scale_y, offset_x, offset_y, rotation_angle, grid_adjustment]
    ...
    if (opt.use_STN != 1) and (opt.use_learnable_scales != 1):
        print("Using Neither STN nor Learnable Scales")
        optimizer = torch.optim.Adam([found_zernike_coef, found_object], lr=learning_rate)
    elif (opt.use_STN == 1) and (opt.use_learnable_scales != 1):
        print("Using STN but NOT using Learnable Scales")
        for param in stn_params:
        ...
        print("NOT using STN but using Learnable Scales")
        ...
        print("Using STN and Learnable Scales")
        for scale_factor in scale_factors_list:
            scale_factor.requires_grad_()
        for param in stn_params:
            param.requires_grad_()
    ...
    for epoch_idx in range(n_epochs):
        ...
        for pair_idx in tqdm(range(len(coeffs)), desc="epoch {} optimization".format(epoch_idx)):
            ...
            found_coeffs_all_fovs = found_zernike_coef + found_zernike_coef_diff_fovs
            ...
            phases_on_dm = torch.einsum("ij, jkl -> ikl", 1e-6 * found_coeffs_all_fovs, zernikes)
            if opt.use_learnable_scales:
            ...
            total_phases = phases_on_dm + rotated_applied_phase
            ...
            #### divide FOVs in the different objects:
            object = found_object.unsqueeze(0).unsqueeze(0).clone()
            # skio.imsave(base_dir + "/{}/found_objects/epoch_{}/found_object_{}_pair_idx_{}.tif".format(exp_name, epoch_idx, 0, pair_idx), found_object.detach().cpu().numpy())

            if opt.use_2photon == 1:
                psfs, aberrated_imgs_combined, img_patches, output_patches = spatially_varying_forward_overlap_save_2photon(amplitude, beam_phases_scaled, object, ks, stride_w, stride_h, k)
            else:
                psfs, aberrated_imgs_combined, img_patches, output_patches = spatially_varying_forward_overlap_save(amplitude, beam_phases_scaled, object, ks, stride_w, stride_h, k)
            
            if (pair_idx == 0) and (epoch_idx % 30 == 0):
                for i in range(num_patches_h*num_patches_w):
                    iPSF_stack[i].append(psfs[i, :, :].detach().cpu())
                # if epoch_idx % 10 == 1:
                skio.imsave(base_dir + "/{}/found_objects/object_patches.tif".format(exp_name), output_patches.detach().cpu().numpy())
                skio.imsave(base_dir + "/{}/found_objects/phases.tif".format(exp_name), beam_phases_scaled.detach().cpu().numpy())




            generated_imgs.append(aberrated_imgs_combined.squeeze().detach().cpu().numpy())
            
            observed_img = phase_added_images[pair_idx]
            
            recon_observed_img_FT = torch.real(torch.fft.fft2(aberrated_imgs_combined))
            ...
            if opt.use_L1_loss == 1:
                ...
                recon_loss = L1_loss(aberrated_imgs_combined.squeeze(), observed_img) #+ 0.1*Fourier_loss # L1_loss mse_loss
            else:
                Fourier_loss = mse_loss(recon_observed_img_FT.squeeze(), observed_img_FT)
                recon_loss = mse_loss(aberrated_imgs_combined.squeeze(), observed_img) #+ 0.1*Fourier_loss # L1_loss mse_loss


            varying_zernikes_L1_norm = L1_loss(found_zernike_coef_diff_fovs, torch.zeros_like(found_zernike_coef_diff_fovs))

            loss = recon_loss +  opt.spatially_varying_weight * varying_zernikes_L1_norm+ opt.fourier_loss_weight * Fourier_loss
            ...
            # print("difference in gradient:", torch.sum(found_object.grad - object.grad))
            ## zero out the loss to the tip/tilt coefficients
            found_zernike_coef.grad.mul_(mask)
            ### not sure if this will work
            found_zernike_coef_diff_fovs.grad.mul_(mask)

            ## Projection:
            # print(found_object.grad.max(), found_object.grad.min())
            ...
            with torch.no_grad():
                ...
                # found_object.data = found_object.data * ( img_energy / torch.sum(found_object).clamp(min=1e-6) )


            epoch_loss += recon_loss.item()
            epoch_fourier_loss += Fourier_loss.item()
            epoch_zernikes_varying_L1 += varying_zernikes_L1_norm.item()

        current_time = time.time()
        # print(found_zernike_coef)
        ...
        # forward_pass_time /= len(coeffs)
        forward_pass_times_epoch.append(forward_pass_time)
        ...
        if epoch_idx == opt.epochs - 1 or (opt.use_stopping_condition and stop):
            ...
            gt_stn_params = [opt.gt_amplitude_scale_x, opt.gt_amplitude_scale_y, opt.gt_offset_x, opt.gt_offset_y]
            np.savetxt(base_dir + "/{}/final_results/gt_stn_params.txt".format(exp_name), gt_stn_params)
            ...
            np.savetxt(base_dir + "/{}/final_results/stn_params.txt".format(exp_name), params_save)
        ...
        if epoch_idx % 10 == 1:
            ...
            # for key, items in iPSF_stack.items():
            #     # print("key", key)
            #     iPSF_stack_save = torch.stack(items, 0).numpy()
            #     skio.imsave(base_dir + "/{}/iPSFs/psf_{}.tif".format(exp_name, key), iPSF_stack_save)
            # iPSF_stack_save = torch.stack(iPSF_stack, 0).numpy()
            ...
            np.savetxt(f"{base_dir}/{exp_name}/time_at_epoch.txt", np.array([epoch_idx, time_at_epoch]))

            np.savetxt(f"{base_dir}/{exp_name}/forward_pass_times.txt", np.array(forward_pass_times_epoch))

        
        if epoch_idx % 10 == 0:

            np.savetxt(base_dir + "/{}/zernikes_saved/common_mode/found_zernikes_{}.txt".format(exp_name, epoch_idx), np.array(found_zernike_coef.detach().cpu().numpy()))
            np.savetxt(base_dir + "/{}/zernikes_saved/varying/found_zernikes_all_fovs_{}.txt".format(exp_name, epoch_idx), np.array(found_zernike_coef_diff_fovs.detach().cpu().numpy()))
            ...
            plt.title("Zernikes Varying L1 Loss", fontdict={'fontsize': 12})
            plt.xlabel("epochs")
            plt.ylabel("loss")
            ...
            params_save = [amplitude_scale_x.item(), amplitude_scale_y.item(), offset_x.item(), offset_y.item(), rotation_angle.item(), grid_adjustment.item()]
            np.savetxt(base_dir + "/{}/stn_params_{}.txt".format(exp_name, epoch_idx), params_save)
